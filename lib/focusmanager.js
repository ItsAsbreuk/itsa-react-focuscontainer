...
const tabIndexesOrder = (a, b) => {
    ...
    if (tabB>tabA) {
        // else put the new item `b` behind previous item `a`
        return -1;
    }
    // else: we need to inspect the data-subtabindex
    subTabA = parseInt(a.getAttribute("data-subtabindex"), 10);
    subTabB = parseInt(b.getAttribute("data-subtabindex"), 10);
    if (subTabB<subTabA) {
        // put the new item `b` before previous item `a`
        return 1;
    }
...
const getFocussableNodes = (container, selector) => {
    ...
    nodes.forEach(node => {
        ...
        if (typeof tabIndex==="string") {
            // set the `data-sub-index` to ensure the right order in case multple items have the same tabIndex:
            SUB_TAB_INDEXES[tabIndex] || (SUB_TAB_INDEXES[tabIndex]=0);
            node.setAttribute("data-subtabindex", ++SUB_TAB_INDEXES[tabIndex]);
...
const defaultFnSetFocus = function(e) {
    ...
    if (node.setAttribute) {
        ...
        return new Promise(resolve => {
            ...
            async(() => {
                if (node.itsa_scrollIntoView && ((node===document.activeElement) || props.scrollIntoView)) {
                    node.itsa_scrollIntoView(null, null, transitionTime, marginTop, marginLeft);
                }
                node.focus();
                resolve(true);
            });
